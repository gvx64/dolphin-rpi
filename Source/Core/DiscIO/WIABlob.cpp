...
{
//gvx64  ASSERT(m_exception_lists == 0);
  _assert_(m_exception_lists == 0); //gvx64 rollforward to 5.0-12188 - implement .rvz support
...
//gvx64  ASSERT(data <= data_start + (m_compressed_exception_lists ? m_out_bytes_used_for_exceptions :
  _assert_(data <= data_start + (m_compressed_exception_lists ? m_out_bytes_used_for_exceptions : //gvx64 rollforward to 5.0-12188 - implement .rvz support
...
//gvx64    if (volume->ReadSwapped<u32>(partition.offset, PARTITION_NONE) != u32(0x10001))
    if ( (volume->template ReadSwapped<u32>(partition.offset, PARTITION_NONE)) != u32(0x10001)) //gvx64 rollforward to 5.0-12188 - implement .rvz support
    {
...
//gvx64    ASSERT(parameters.data.size() % VolumeWii::BLOCK_TOTAL_SIZE == 0);
    _assert_(parameters.data.size() % VolumeWii::BLOCK_TOTAL_SIZE == 0); //gvx64 rollforward to 5.0-12188 - implement .rvz support
...
//gvx64            ASSERT(offset_in_block + sizeof(SHA1) <= VolumeWii::BLOCK_HEADER_SIZE);
            _assert_(offset_in_block + sizeof(SHA1) <= VolumeWii::BLOCK_HEADER_SIZE); //gvx64 rollforward to 5.0-12188 - implement .rvz support
...
//gvx64              ASSERT(hash_offset <= std::numeric_limits<u16>::max());
              _assert_(hash_offset <= std::numeric_limits<u16>::max()); //gvx64 rollforward to 5.0-12188 - implement .rvz support
...
//gvx64    ASSERT((*bytes_written & 3) == 0);
    _assert_((*bytes_written & 3) == 0); //gvx64 rollforward to 5.0-12188 - implement .rvz support
...
//gvx64      StringFromFormat(Common::GetStringT("%i of %i blocks. Compression ratio %i%%").c_str(),
      StringFromFormat(GetStringT("%i of %i blocks. Compression ratio %i%%").c_str(), //gvx64
...
WIARVZFileReader<RVZ>::Convert(BlobReader* infile, const VolumeDisc* infile_volume,
                               ...
                               void* arg)
{
//gvx64  ASSERT(infile->IsDataSizeAccurate());
  _assert_(infile->IsDataSizeAccurate()); //gvx64 rollforward to 5.0-12188 - implement .rvz support
//gvx64   ASSERT(chunk_size > 0);
  _assert_(chunk_size > 0);  //gvx64 rollforward to 5.0-12188 - implement .rvz support
...
//gvx64    ASSERT(groups_processed == first_group);
    _assert_(groups_processed == first_group); //gvx64 rollforward to 5.0-12188 - implement .rvz support
//gvx64    ASSERT(bytes_read == data_offset);
    _assert_(bytes_read == data_offset); //gvx64 rollforward to 5.0-12188 - implement .rvz support
...
//gvx64    ASSERT(data_size == 0);
    _assert_(data_size == 0); //gvx64 rollforward to 5.0-12188 - implement .rvz support
  }

//gvx64  ASSERT(groups_processed == total_groups);
  _assert_(groups_processed == total_groups); //gvx64 rollforward to 5.0-12188 - implement .rvz support
//gvx64  ASSERT(bytes_read == iso_size);
  _assert_(bytes_read == iso_size); //gvx64 rollforward to 5.0-12188 - implement .rvz support
  ...
  if (infile_volume)
  {
//gvx64    if (infile_volume->GetVolumeType() == Platform::GameCubeDisc)
    if (infile_volume->GetVolumeType() == Platform::GAMECUBE_DISC) //gvx64 rollforward to 5.0-12188 - implement .rvz support
      disc_type = 1;
//gvx64    else if (infile_volume->GetVolumeType() == Platform::WiiDisc)
    else if (infile_volume->GetVolumeType() == Platform::WII_DISC)  //gvx64 rollforward to 5.0-12188 - implement .rvz support
      disc_type = 2;
  }
...
//gvx64  std::unique_ptr<VolumeDisc> infile_volume = CreateDisc(infile_path);
  std::unique_ptr<VolumeDisc> infile_volume = CreateDisc(infile_path); //gvx64 rollforward to 5.0-12188 - implement .rvz support
